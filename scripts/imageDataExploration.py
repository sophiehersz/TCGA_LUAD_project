...
def processEmbeddings(path, file_names):
    '''
    Returns dataframe with mean or median embeddings for each sample
...
def plotMAP(df, title=None, hue='mutation'):
    fig, ax = plt.subplots()
    if hue=='mutation':
        sns.scatterplot(data=df, x='x', y='y', hue='mutation', palette=['darkblue', 'red'], s=30)
        plt.legend(loc='upper left', title='STK11 mutation', bbox_to_anchor=(1.01, 1),borderaxespad=0)
        plt.title(title)
    elif hue == 'cluster':
        sns.scatterplot(data=df, x='x', y='y', hue='cluster', s=30, palette='bright')
        plt.legend(loc='upper left', title='expression cluster', bbox_to_anchor=(1.01, 1),borderaxespad=0)
        plt.title(title)
    return(fig)
...
patients = pd.read_csv('ddata/data_for_ML/sample_info_tumor_only_no_outliers.csv') # dataframe containing list of patients to be used
...
pd.DataFrame(mutation_status, columns=['mutation']).to_csv('data/data_for_ML/mutation_status_embeddings.csv', index=False)

# Process embeddings ###############################################################################################
mean_embeddings, median_embeddings = processEmbeddings(path_to_embeddings, file_names)
mean_embeddings.to_csv('data/data_for_ML/mean_image_embeddings.csv', index=False)
median_embeddings.to_csv('data/data_for_ML/median_embeddings.csv', index=False)
...
fig = plotMAP(pca_mean, title='PCA reduction of image embeddings\n(mean values)')
...
fig = plotMAP(pca_median, title='PCA reduction of image embeddings\n(median values)')
...
fig = plotMAP(umap_mean, title='UMAP reduction of image embeddings\n(mean values)')
...
fig = plotMAP(umap_median, title='UMAP reduction of image embeddings\n(median values)')
...
fig = plotMAP(tsne_mean, title='TSNE reduction of image embeddings\n(mean values)')
...
fig = plotMAP(tsne_median, title='TSNE reduction of image embeddings\n(median values)')
